...
export async function savePlant(plant: PlantProps) : Promise<void> {
    try {
        ...
        const nexTimeToday = new Date(plant.dateTimeNotification);

        nexTimeToday.setDate(nexTimeToday.getDate())

        const secondsToday = Math.abs(
            Math.ceil(now.getTime() - nexTimeToday.getTime()) / 1000);
        ...
        await Notifications.scheduleNotificationAsync({
            ...
            trigger: {
                seconds: secondsToday < 60 ? 60 : secondsToday,
            },
        });
